-- TIGHTEN I.110 (2026-06-02): clear stale lost_reason / rejected_at
-- whenever a quote becomes accepted, AND backfill any rows where the
-- markers are out of sync with the linked order's current state.
--
-- Context: smoke walk on Spit Braai Delivery surfaced QT-20260503-7N868C
-- with status='accepted', lost_reason='order_cancelled', AND linked
-- order status='confirmed'. The aggregators bucket this as
-- won_then_cancelled (false-positive churn signal) even though the
-- order is currently in good standing.
--
-- Root cause: convert_quote_to_order RPC + public accept route both
-- stamp accepted_at without clearing lost_reason / rejected_at left
-- behind by a previous (now-reversed) cancellation cycle.
--
-- Two parts to the fix:
--   1. Override convert_quote_to_order with a version that clears
--      lost_reason + rejected_at when flipping status to 'accepted'.
--   2. One-shot backfill: any accepted quote whose linked order is
--      currently NOT cancelled but which carries lost_reason=
--      'order_cancelled' is stale - clear both markers.

-- ── 1. Re-create convert_quote_to_order with the clearing UPDATE ──
...
GRANT EXECUTE ON FUNCTION public.convert_quote_to_order(uuid, uuid, uuid, jsonb)
  TO authenticated, service_role;

-- ── 2. Backfill: clear stale markers where the linked order is in
--      good standing today. Idempotent. ──
DO $$
DECLARE
...
BEGIN
  ...
  WHERE q.status = 'accepted'
    AND q.lost_reason = 'order_cancelled'
    AND q.converted_to_order_id IS NOT NULL
    AND EXISTS (
      SELECT 1
  ...
  RAISE NOTICE '[TIGHTEN I.110] Cleared stale lost_reason/rejected_at on % accepted quote rows whose linked order is in good standing', rows_updated;
END $$;
