...
CREATE TABLE public.profiles (
  ...
  CHECK (role IN ('company_admin', 'client', 'driver', 'kitchen_staff', 'cleaning_staff', 'shopping_staff', 'super_admin'));
);
...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (id, email, full_name, role, currency, phone_number, company_name, subscription_status, subscription_plan, trial_ends_at, is_active)
  VALUES (
    NEW.id,
    NEW.email,
    ...
    'trial', -- Default status on user creation
...
CREATE TABLE public.cleaning_schedules (
  ...
  -- Schedule details
  area_name TEXT NOT NULL, -- 'kitchen_staff', 'storage', 'vehicle_fleet', 'equipment_storage'
  description TEXT,
...
-- 
-- Questions? Consult the architectural decisions in the Executive