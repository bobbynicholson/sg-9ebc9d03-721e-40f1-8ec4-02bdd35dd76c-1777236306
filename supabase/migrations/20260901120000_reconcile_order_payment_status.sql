...
AS $$
DECLARE
  -- Use named variables instead of a RECORD for the order snapshot. A
  -- RECORD's fields are inferred from the SELECT shape at compile time;
  -- older deployed definitions of this function were missing
  -- balance_amount and failed during the backfill with "record v_order
  -- has no field balance_amount".
...
BEGIN
  ...
  IF NOT v_has_ledger THEN
    v_paid := GREATEST(
      ...
      CASE
        WHEN COALESCE(v_deposit_paid_existing, false)
          THEN COALESCE(v_deposit_amount, 0)
        ELSE 0
      END,
      CASE
        WHEN COALESCE(v_balance_paid_existing, false)
          THEN COALESCE(v_balance_amount_existing, 0)
        ELSE 0
      END,
      CASE
        WHEN v_payment_status_existing = 'paid'
          THEN COALESCE(v_total_amount, 0)
        ELSE 0
      END
    );
  END IF;
