-- Wave 30.1: client_view_order RPC was crashing with 22P02 every
-- time a magic-link client opened their order link.
--
-- Same bug class as Wave 28.9 (orderWorkflow cancel cascade): the
-- RPC compared invoices.status against the literal 'cancelled'
-- which doesn't exist in the invoice_status enum
-- {draft|sent|paid|partially_paid|overdue|written_off}. Postgres
-- failed the cast on every single call -- so every "Client link"
-- and "View as client sees it" admin action shipped a token URL
-- that immediately threw "Lookup failed" on the public page.
--
-- Fix: use written_off (the actual enum value the cancel cascade
-- now writes), and use NOT IN so the predicate covers both the
-- already-paid and voided buckets in one go.
--
-- Behaviour preserved: still excludes soft-deleted, paid, and
-- voided invoices from the "live invoice with money outstanding"
-- lookup the magic-link page uses to render the Pay button.
...
BEGIN
  ...
  -- Wave 30.1 fix: was `status <> 'cancelled'` which crashed because
  -- the invoice_status enum has no 'cancelled' value. NOT IN
  -- (paid, written_off) covers the same intent (skip already-settled
  -- and voided invoices) and is enum-safe.
