-- Expose a true paid-to-date figure on the order returned by
-- client_view_order, so the client magic-link page (/c/order/[id].tsx,
-- commit 69fac694) can render an accurate "deposited / remaining"
-- breakdown.
--
-- Why: 69fac694 reads `order.amount_paid` first ("prefer the ledger
-- figure ... so the deposited figure is never silently 0 after a real
-- payment") and only falls back to the deposit_paid / balance_paid
-- flags. But the order SELECT in client_view_order never returned
-- amount_paid, so that preferred branch was dead. For payments taken
-- through the invoice flow (record_invoice_payment sets
-- orders.payment_status='partial' but does NOT set deposit_paid), the
-- breakdown then showed a contradictory "Deposit paid" header with
-- "Amount paid R0 / Remaining = full total".
--
-- The real ledger lives on invoices.amount_paid (record_invoice_payment
-- accumulates it there). orders.amount_paid is not maintained and
-- payments rows from the invoice flow carry no order_id, so neither is
-- a reliable source. We therefore sum invoices.amount_paid for the
-- order.
--
-- Non-regressing: NULLIF(...,0) leaves amount_paid NULL when the order
-- has no invoice payments, so the client keeps falling back to the
-- deposit/balance flags exactly as before. Only when there is a real
-- invoice-recorded payment does the accurate figure take over.
--
-- Verbatim copy of 20260518704900_wave70_49b with two additions:
--   1. DECLARE v_paid numeric;
--   2. compute v_paid + merge it into the 'order' object in the RETURN.
...
BEGIN
  ...
  -- True paid-to-date across ALL of the order's invoices (any status).
  -- NULL when there are none, so the client falls back to the
  -- deposit/balance flags exactly as before this change.
  SELECT NULLIF(COALESCE(SUM(amount_paid), 0), 0)
    INTO v_paid
    FROM invoices
    WHERE order_id = p_order_id
      AND deleted_at IS NULL;
  ...
  RETURN jsonb_build_object(
    'ok',                  true,
    'order',               to_jsonb(v_order) || jsonb_build_object('amount_paid', v_paid),
    'items',               v_items,
