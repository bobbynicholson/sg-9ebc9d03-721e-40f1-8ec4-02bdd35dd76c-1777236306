-- client_view_order: include equipment names, quantities and prices in the
-- public tracking payload.
--
-- WHY: /c/order/[id] already receives equipment_bookings for timeline state,
-- but the payload only exposed status + returned_quantity. The customer
-- tracking page could not show which equipment contributed to the accepted
-- quote/order. This keeps the existing function shape and enriches only the
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           ...
           'name', COALESCE(eq.name, 'Equipment'),
           'quantity', eb.quantity,
           'unit_price', eq.rental_price,
           'line_total', COALESCE(eq.rental_price, 0) * COALESCE(eb.quantity, 0),
           'is_hire_in', eq.is_hire_in,
           'status', eb.status,
           'returned_quantity', eb.returned_quantity
         ) ORDER BY COALESCE(eq.name, 'Equipment')), '[]'::jsonb)
  INTO v_bookings
  FROM equipment_bookings eb
  LEFT JOIN equipment eq ON eq.id = eb.equipment_id
