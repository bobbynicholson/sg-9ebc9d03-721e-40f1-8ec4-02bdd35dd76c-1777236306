-- Expose orders.deposit_percentage on the order returned by
-- client_view_order so the client magic-link page (/c/order/[id].tsx)
-- can show the deposit split (e.g. "Deposit (50%)") and derive the
-- deposit/remaining amounts even when deposit_amount / balance_amount
-- were never stamped on the order.
--
-- Why: a quote accepted without a captured-at-accept deposit leaves
-- orders.deposit_amount / balance_amount NULL, but the configured
-- deposit share still lives on orders.deposit_percentage (carried from
-- the quote / companies.deposit_percent, default per tenant). The order
-- SELECT in client_view_order never returned deposit_percentage, so the
-- page had nothing to render and the deposit line silently disappeared.
--
-- Verbatim copy of 20260620120000 with one addition: deposit_percentage
-- in the order SELECT list (v_order is a RECORD, so to_jsonb(v_order)
-- only carries the columns explicitly selected).
...
BEGIN
  ...
  SELECT
    ...
    deposit_paid_at, deposit_percentage, balance_amount, balance_paid,
    balance_paid_at, balance_due_date, currency, client_name,
    client_email, client_phone, company_id, created_at, confirmed_at,
    ready_at, picked_up_at, delivered_at, completed_at, departed_venue_at,
