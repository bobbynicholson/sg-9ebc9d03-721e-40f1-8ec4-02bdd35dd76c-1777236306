-- Wave 60 -- client_view_order returns the full money breakdown +
-- timeline-relevant fields + related rows so the public client page
-- shows everything the admin sees (subtotal / discount / delivery /
-- VAT / total) and renders the same timeline stages with the same
-- statuses (driven by payments / driver_assignments / kitchen_prep
-- / equipment_bookings rather than the order row alone).
