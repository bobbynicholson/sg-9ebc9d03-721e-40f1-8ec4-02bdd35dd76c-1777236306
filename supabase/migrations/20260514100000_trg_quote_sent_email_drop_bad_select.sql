-- Wave 16: drop the email_automation_log.quote_id dedupe SELECT from
-- trg_quote_sent_email. Wave 10B added that SELECT to catch the
-- in-app QuoteSendDialog vs DB-trigger double-fire on quote.sent
-- emails, but email_automation_log doesn't have a quote_id column
-- (it has user_id, order_id, template_type, recipient_email, status,
-- error_message, sent_at, etc. -- no quote linkage). The SELECT
-- 500's with "column email_automation_log.quote_id does not exist",
-- which rolls back EVERY quote UPDATE that transitions status to
-- 'sent'. Operators saw "Save failed: column 'quote_id' does not
-- exist" on Save & Send.
--
-- The outgoing_email_queue dedupe check above the bad SELECT is
-- enough to prevent the double-fire on its own (it's keyed on
-- trigger_event='quote.sent' AND trigger_ref_id=NEW.id, which is
-- exactly what the in-app dialog also writes to). Drop the bad
...
BEGIN
  ...
  -- Dedupe via outgoing_email_queue. The in-app QuoteSendDialog +
  -- this trigger BOTH write a row tagged trigger_event='quote.sent'
  -- + trigger_ref_id=NEW.id, so the same EXISTS check catches both
  -- paths and prevents the double-fire on its own.
  IF EXISTS (
