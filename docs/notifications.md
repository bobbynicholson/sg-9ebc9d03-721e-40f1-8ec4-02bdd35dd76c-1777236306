...
6. ~~**Per-recipient suppression list.**~~ Done. The check side was already wired (`emailService.sendEmail` consults `blocked_contacts` before delivery). Post-audit added the public side: `src/lib/emailUnsubscribe.ts` mints HMAC-signed deterministic tokens; `/api/public/email-unsubscribe` upserts into `blocked_contacts` on confirm; `/u/[token]` is the confirmation page. Email templates should use `buildUnsubscribeUrl(baseUrl, email, companyId)` to embed the link in footers - rollout across templates is the mechanical follow-up.
