...
8. ~~Bulk import of supplier payables (admin audit gap).~~ Done. `/admin/payables` now has a "Bulk import" dialog accepting pasted CSV (`supplier_name, amount, due_date, invoice_ref, notes`). Header row auto-detected, currency symbols stripped, dates parse ISO + DD/MM/YYYY + YYYY/MM/DD. Preview shows per-row match status (matched supplier / ready / error reason) before commit. Sequential inserts so per-row failures are reported in the result panel without losing the rest of the batch.
