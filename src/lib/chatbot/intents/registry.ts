...
export const CORE_CHAT_INTENTS: readonly ChatIntentDefinition[] = [
  ...
  { id: "kitchen.schedule.upcoming", domain: "kitchen", action: "read", entity: "schedule", roles: ["kitchen_manager", "kitchen_staff"], toolIds: ["kitchen_orders", "kitchen_prep_tasks"], navigationRefs: ["kitchen.today", "kitchen.production", "kitchen.stock"], phrases: ["upcoming events", "upcoming orders", "future kitchen work", "next kitchen events"], keywords: ["upcoming", "future", "events", "orders", "schedule", "work", "prep", "production"], patterns: [/\b(?:upcoming|future)\b/, /\b(?:next|coming)\b[\s\S]*\b(?:event|order|work|prep|production)\b/], priority: 99, description: "Upcoming kitchen events, orders, and preparation work." },
  { id: "kitchen.schedule.today", domain: "kitchen", action: "read", entity: "schedule", roles: ["kitchen_manager", "kitchen_staff"], toolIds: ["kitchen_orders", "kitchen_prep_tasks"], navigationRefs: ["kitchen.today", "kitchen.production", "kitchen.stock"], phrases: ["today's kitchen work", "today's prep", "events today", "today's orders", "what do i have today"], keywords: ["today", "events", "orders", "work", "prep", "production", "schedule"], patterns: [/\b(?:today|current)\b[\s\S]*\b(?:event|order|work|task|prep|production|schedule)\b/], priority: 98, description: "Today's kitchen orders and prep tasks." },
