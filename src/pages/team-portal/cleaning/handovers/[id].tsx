 ...
 *
 * Command-centre restructure: chrome now comes from CleaningPageShell
 * (hero, workbench, footer); the BookingHeader below stays as the
 * canonical event-document header inside the body.
 */
...
import { ArrowLeft, CheckCircle2, Package, PackageOpen, Loader2, AlertTriangle, ExternalLink, RefreshCw, ClipboardList } from "lucide-react";
...
function HandoverDetailInner() {
  ...
  // Command-centre standard: a failed primary read renders a rose
  // recovery card with Retry, never a silent "not found".
  ...
  const doneJobs = jobs.filter((j) => j.status === "complete" || j.status === "cancelled").length;
  const chipsReady = !loading && !loadError && !!handover;

  return (
    <CleaningPageShell
      pageTitle={`Cleaning handover ${handover?.order_number || ""} - CateringMS`}
      heading="Cleaning handover"
      subheading={
        chipsReady
          ? [handover?.order_number ? `Order ${handover.order_number}` : null, handover?.client_name || null]
              .filter(Boolean)
              .join(", ") || "Per-event equipment return, washing jobs and sign-off."
          : "Per-event equipment return, washing jobs and sign-off."
      }
      icon={PackageOpen}
      meta={
        chipsReady ? (
          <>
              ...
              <span className={CLEANING_HERO_CHIP}>
                <span
                  className={cn(
                    ...
                    handover?.status === "complete" ? "bg-emerald-400" : handover?.status === "cancelled" ? "bg-slate-400" : "bg-amber-400",
                  )}
                />
                {phaseMeta.label}
              </span>
            ...
            {jobs.length > 0 && (
              <span className={CLEANING_HERO_CHIP}>
                ...
                {doneJobs}/{jobs.length} jobs done
              </span>
            )}
          </>
        ) : undefined
    ...
    >
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-rose-200 bg-rose-50 p-5 shadow-sm dark:border-rose-900/60 dark:bg-rose-950/40">
          <h2 className="text-base font-bold text-rose-900 dark:text-rose-200 mb-1">Couldn&apos;t load this handover</h2>
        ...
        <Card><CardContent className="py-16 text-center text-slate-500">
          <p className="font-medium text-slate-900 dark:text-white mb-1">Handover not found</p>
          <p className="text-sm mb-4">It may have been removed, or the link is for a different company.</p>
          <Button asChild size="sm" variant="outline">
            <Link href={withSlug("/team-portal/cleaning/dashboard")}>Back to the cleaning dashboard</Link>
          </Button>
        </CardContent></Card>
      ) : (
        <>
              ...
              <div className="flex items-center gap-2">
                ...
                {handover.order_id && (
                  <Link
                    ...
                    className="inline-flex items-center gap-1 text-[11px] text-brand-primary hover:text-brand-primary/80 hover:underline bg-brand-primary/5 border border-brand-primary/20 rounded px-2 py-0.5"
                ...
                <Badge variant="outline" className="bg-brand-primary/5 text-brand-primary border-brand-primary/20 text-[11px] gap-1">
            ...
            <Card className="border-0 shadow-sm mb-4">
              <CardContent className="px-4 py-2.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px]">
                {handover.expected_at && handover.status === "expected" && (
                  <span className="text-brand-primary">Expected back by {fmtDateTime(handover.expected_at)}</span>
                )}
                        ...
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          ...
                          {canManageCleaning && (j.status === "queued" || j.status === "in_progress") && (
                            <Button
                              ...
                              className="bg-brand-primary hover:bg-brand-primary/90 text-xs h-8 gap-1"
                            >
