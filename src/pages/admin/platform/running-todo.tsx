...
const productNotesCards: SprintCard[] = [
  ...
  },
  {
    ...
    defaultOpen: true,
    items: [
      { title: "Owner-editable 'cash on hand' field on /admin/financial-dashboard", detail: "New card at the top of the dashboard with an inline-editable number the owner can punch in any time (typically updated from their bank balance once a day). Persisted to a per-company row (probably a new companies.cash_on_hand_cents + companies.cash_on_hand_updated_at column) so it survives reload and feeds every other surface that needs the figure. Owner / director role only - never visible to staff (Skylight finance-visibility rule).", status: "todo" },
      { title: "Forecast number: cash on hand + income still to come - costs still to come", detail: "Second number on the same card. Pulls (a) sum of deposits + balances expected on confirmed orders with event_date in the forecast window; (b) sum of scheduled costs (staff wages, supplier payables, recurring fixed costs); (c) projected variable costs (kitchen prep cost from inventoryDeductionService, predicted shopping run cost from kitchenPrepService aggregated demand). Net = cash_on_hand + (a) - (b) - (c). Colour codes positive (green) / tight (amber) / negative (red).", status: "todo" },
      { title: "Forecast window picker (7 / 14 / 30 / 60 / 90 days)", detail: "Owner picks the horizon. Default 30 days. Window affects what 'income still to come' + 'costs still to come' sum over. Same picker pattern as the existing /team-portal/shopping/kitchen-demand horizon selector so it's familiar.", status: "todo" },
      { title: "Interactive cashflow chart below the card (Recharts area + line)", detail: "Day-by-day or week-by-week area chart of projected running balance over the forecast window. Hovering a point shows the breakdown for that day: opening balance, income in (per order), costs out (per category: wages / shopping / suppliers). Click a point opens a detail drawer with the contributing rows. Interactive enough that the owner can simulate 'what if I push this big balance payment a week earlier'.", status: "todo" },
      { title: "Cost categories the forecast counts: staff wages, supplier payables, recurring fixed costs, predicted shopping, hired equipment", detail: "Staff wages from staff_shifts / staff_payments due in the window. Supplier payables from supplier_invoices.due_date. Recurring costs from a new fixed_costs table (rent, software, vehicles). Predicted shopping from kitchenPrepService aggregated demand x inventory.unit_cost. Hired equipment from equipment_hire_orders. Each category collapsible in the detail drawer so the owner can see which line items are eating the balance.", status: "todo" },
      ...
      { title: "'Probable spend' override input per category", detail: "Some costs can't be derived (e.g. last-minute shopping, fuel, an emergency rental). Owner can punch in a flat amount per category for the forecast window to nudge the prediction up - effectively a manual 'contingency' line. Persisted so it doesn't reset on reload.", status: "todo" },
      { title: "Already-scheduled costs roll into the same card", detail: "Anything already on the books with a due_date inside the window (a confirmed supplier invoice, a staff wage commitment, an equipment hire) appears in the cost side of the forecast. The card surfaces both the live cash-on-hand number AND the forward-looking forecast so the owner sees them side by side.", status: "todo" },
      { title: "Visibility gate: director / owner role only", detail: "Per Skylight finance-visibility rule (memory: finance info stays in Directors folder only), this card never renders for kitchen / driver / shopping / admin / company_admin. Gated by useAuth().profile.role === 'owner' (and super_admin for support). Same pattern as the existing booked-revenue tile removal in Wave 56.", status: "todo" },
      { title: "Audit trail: log every cash_on_hand edit + every forecast snapshot to audit_logs", detail: "Every time the owner updates the cash-on-hand figure, write an audit_logs row (action='financial.cash_on_hand.update', details: { old, new, by }). On demand the owner can export a CSV of the daily forecast snapshot for the bookkeeper. Closes the loop with the existing audit_logs / Skylight Directors finance separation.", status: "todo" },
      { title: "Future: hook into Xero / QuickBooks balance feed for auto-update", detail: "Phase 2: instead of the owner typing the bank balance daily, pull it from the connected accounting integration's bank-feed endpoint nightly. Removes the last manual step. Depends on the Xero token-refresh gap (Phase 2E in audit) being closed first.", status: "todo" },
    ],
  },
  {
  ...
  return (
    <>
      ...
      <main className="min-h-screen overflow-x-hidden bg-gradient-to-br from-slate-50 via-white to-purple-50 lg:pl-64 xl:pl-72 pt-16 lg:pt-0">
        <div className="px-3 sm:px-4 md:px-6 pt-20 lg:pt-6 pb-8 max-w-screen-2xl">
          <div className="mb-6">
