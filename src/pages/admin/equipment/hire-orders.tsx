 ...
 *
 * Auto-generated procurement checklist. When a quote flips to
 * 'accepted' AND it includes equipment lines that overcommitted past
 * the catering company's stock (from_hire_qty > 0), the
 * trg_quote_accepted_spawn_hire_orders trigger drops one row in
 * equipment_hire_orders for each shortfall line.
 *
 * The team's job here:
 *   - Pick a supplier
 *   - Set expected pickup + return dates
 *   - Mark pickup + return as the day arrives
 *   - Watch the running cost so margin doesn't slip
 *
 * Tenant-scoped via RLS. Status workflow:
 *   draft -> confirmed -> picked_up -> returned   (or cancelled)
 */
...
import {
  ArrowLeft, Package, Calendar as CalendarIcon, Truck, CheckCircle2,
...
function HireOrdersPage() {
  ...
  // Slug-aware deep-link prefix for "open the parent order" CTA. Reads
  // the current pathname's first segment (set by the next.config rewrite
  // when the user is on /[slug]/admin/...).
        ...
        <div className="px-4 py-8 max-w-screen-2xl mx-auto">
          <Link href={`${slugPrefix}/admin/equipment`}>
            <Button variant="ghost" size="sm" className="mb-4">
              <ArrowLeft className="w-4 h-4 mr-2" /> Back to equipment catalog
            </Button>
          </Link>
