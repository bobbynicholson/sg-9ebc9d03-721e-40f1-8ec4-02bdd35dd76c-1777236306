...
export function ShoppingNav({ className, companySlug }: ShoppingNavProps) {
  ...
  const allHrefs = shoppingNavSections.flatMap((s) => s.items.map((i) => i.href));
  const isActive = buildIsActive(allHrefs, { router, withSlug });

  const desktopScrollRef = useNavScrollRestore<HTMLDivElement>("shopping-nav");
        ...
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            <Sheet open={open} onOpenChange={setOpen}>
              ...
              >
                <div
                  className="px-6 py-4 border-b bg-gradient-to-r from-green-500 to-emerald-500 flex-shrink-0"
