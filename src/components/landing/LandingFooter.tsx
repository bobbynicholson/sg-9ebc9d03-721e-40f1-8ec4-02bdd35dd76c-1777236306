...
import { ChefHat, Phone, MapPin, ArrowRight, Facebook, Linkedin, Instagram, Sparkles } from "lucide-react";

const PHONE_DISPLAY = "+27 83 652 5755";
const PHONE_TEL = "+27836525755";

// Warm, premium footer for EVERY marketing page. The homepage mounts it
// directly; the shared <Footer /> delegates its marketing variant here so
// the whole public site closes on one voice. Dark stone to bookend the
// dark sections and feel high-end.
//
// White-label aware: a tenant's name/logo replace the CateringMS brand,
// the CateringMS-specific contact/social/CTA blocks drop away, and the
// bottom bar carries the "Powered by CateringMS" attribution instead.
  ...
  return (
    <footer className="bg-stone-950 text-stone-300">
      <div className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
          ...
          <div className="lg:col-span-2">
            <Link href="/" className="flex items-center gap-2">
              {logoUrl ? (
                ...
                <img src={logoUrl} alt={displayName} className="h-9 max-w-[180px] object-contain" />
              ) : (
            ...
            <p className="mt-4 max-w-sm text-sm leading-relaxed text-stone-400">
              {isWhiteLabeled
                ? "Complete catering management - quotes, kitchen, delivery and payments, connected."
                : "The complete operating system for South African catering businesses - quote, plan, deliver and get paid, beautifully, from one platform."}
            </p>
            {!isWhiteLabeled && (
        ...
        {/* CTA strip (platform only - a tenant footer shouldn't sell CateringMS) */}
        {!isWhiteLabeled && (
        ...
        <div className="mt-10 flex flex-col items-center justify-between gap-3 border-t border-white/10 pt-8 text-sm text-stone-500 sm:flex-row">
          <p>
            © {year} {displayName}
            {!isWhiteLabeled && " · A product of Skylight Digital"}
          </p>
          {isWhiteLabeled ? (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 transition-colors duration-150 hover:text-amber-400"
            >
