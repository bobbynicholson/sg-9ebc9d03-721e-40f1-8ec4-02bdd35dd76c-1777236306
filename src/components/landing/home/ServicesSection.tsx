...
import { IMG, Photo, SectionHeader, warmCard, chip } from "./shared";
...
export function ServicesSection() {
  return (
    <section id="services" className="mx-auto max-w-7xl scroll-mt-24 px-4 py-20 md:py-28">
      <SectionHeader
        title="Built for the way you actually cater"
        copy="Whatever you're plating up this weekend, CateringMS runs the operation behind it."
      />

      {/* Two wide photo-led cards per row: the photography carries the
          section, the copy sits roomier underneath. */}
      <Stagger className="grid gap-6 sm:grid-cols-2">
        {SERVICES.map((service, index) => (
          <StaggerItem key={index}>
            <div className={`${warmCard} flex flex-col`}>
              <Photo src={service.img} alt={`${service.title} catering`} gradient={service.gradient} className="aspect-[16/9] w-full" zoom>
                <div className="absolute inset-0 bg-gradient-to-t from-stone-900/50 to-transparent" />
                <div className={`${chip} absolute left-5 top-5 h-11 w-11 bg-white`}>
                  <service.icon className="h-5 w-5 text-amber-700" />
                </div>
              </Photo>
              <div className="flex flex-1 flex-col p-7">
                <h3 className="mb-2 font-display text-2xl font-medium text-stone-900">{service.title}</h3>
                <p className="max-w-[60ch] text-[15px] leading-relaxed text-stone-600">{service.body}</p>
              </div>
            </div>
          </StaggerItem>
        ))}
