 ...
 * localStorage today; the actual sending hookup lands when the backend
 * mailer is wired.
 *
...
export function AutomationSettingsPanel() {
  ...
  const [automationRules, setAutomationRules] = useState<AutomationRule[]>(DEFAULT_RULES);
  ...
  useEffect(() => {
    ...
    if (savedConfig) {
      try { setEmailConfig(JSON.parse(savedConfig)); } catch { /* noop */ }
    }
    const savedRules = localStorage.getItem("automationRules");
    if (savedRules) {
      try { setAutomationRules(JSON.parse(savedRules)); } catch { /* noop */ }
    }
  }, []);
            ...
            {currentRule && (
              <Card className="lg:col-span-2">
                <CardHeader>
                  ...
                  <CardDescription>
                    Customise the email template and settings
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  <div className="flex items-center justify-between">
