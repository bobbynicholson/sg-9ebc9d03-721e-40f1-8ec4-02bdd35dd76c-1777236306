...
export const driverConfirmationService = {
  ...
  /** Close the delivery-leg shift only after the driver is back at base. */
  async markReturnedToBase(orderId: string, driverId: string, location?: { lat: number; lng: number }) {
    ...
    const { data, error } = await (supabase as any)
      ...
      .insert([{
        order_id: orderId,
        driver_id: driverId,
        confirmation_type: "returned_to_base",
        location_lat: location?.lat,
        location_lng: location?.lng,
        confirmed_at: nowIso,
      }])
    ...
    try {
      ...
      if (companyId) {
        ...
        void notificationService.broadcastNotification({
          ...
          title: `Driver back at base: ${(orderRow as any)?.order_number || orderId}`,
          message: "The driver reached the kitchen/warehouse and stopped the shift timer.",
        ...
        }).catch((notifyErr) => console.warn("[markReturnedToBase] admin notification failed:", notifyErr));
      }
    } catch (shiftErr) {
      console.warn("[markReturnedToBase] autoClockOut failed (non-blocking):", shiftErr);
    }
    ...
    try {
      ...
      try {
        await notificationService.broadcastNotification({
          ...
          type: "collection_at_venue",
          title: `Driver arrived to collect: ${o.order_number || orderId}`,
          message: `The driver is at ${venue} collecting equipment for ${eventName}.`,
        ...
        console.warn("[notifyCollectionArrival] kitchen ping failed (non-blocking):", kitchenErr);
      }
      ...
      if (ord) {
        ...
        try {
          await notificationService.broadcastNotification({
            ...
            title: `Equipment collected: ${o.order_number || orderId}`,
            message: `Equipment for ${eventName} has been collected from ${venue} and is returning to base.`,
            targetRoles: [...ADMIN_DISPATCH_ROLES, "kitchen_manager" as any, "kitchen_staff" as any],
            managerDispatch: true,
            priority: "normal",
            link: `/order/${orderId}?role=admin`,
            relatedEntityType: "order",
          ...
          console.warn("[markEquipmentCollected] operations ping failed (non-blocking):", opsErr);
        }
      }
    } catch (notifyErr) {
    ...
    try {
      ...
      try {
        ...
        if (ord) {
          ...
          await notificationService.broadcastNotification({
            companyId: o.company_id,
            type: "collection_en_route",
            title: `Collection started: ${o.order_number || orderId}`,
            message: `The driver is on the way to ${venue} to collect equipment for ${eventName}.`,
            targetRoles: [...ADMIN_DISPATCH_ROLES, "kitchen_manager" as any, "kitchen_staff" as any],
            managerDispatch: true,
            priority: "normal",
            link: `/order/${orderId}?role=admin`,
