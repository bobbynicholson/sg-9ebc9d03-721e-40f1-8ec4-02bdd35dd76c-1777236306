  ...
  const fetchCounts = useCallback(async () => {
    ...
    try {
      ...
      const quotesOverdueQ = sb
        ...
        .select("id", { count: "exact", head: true })
        .eq("company_id", companyId)
        // TIGHTEN I.75: only 'sent' is the live "awaiting client reply"
        // state - draft hasn't gone out yet so it doesn't count.
        .in("status", ["sent"])
      ...
      const revenueQ = canSeeFinance
        ? (() => {
            const q = sb
              ...
              .eq("company_id", companyId)
              .eq("event_date", todayIso)
              // order_status carries workflow progress only - payment
              // state lives on payment_status. Keep the values that
              // genuinely indicate the event is happening or already done.
